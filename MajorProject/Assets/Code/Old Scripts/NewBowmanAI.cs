...
public class NewBowmanAI : MonoBehaviour {
	...
	void SearchandFollow()
	{
		...
		if (Vector3.Distance (player.position, this.transform.position) > Following) {
			...
			if (direction.magnitude < Attacking) {
				...
				arrowSpawnPoint.SetActive (true);

			} else {
				arrowSpawnPoint.SetActive (false);
		...
		} else if (Vector3.Distance (player.position, this.transform.position) < Following) {
