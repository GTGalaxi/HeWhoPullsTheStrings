...
public class SwordsmanAIPatrol : MonoBehaviour {
	...
	void SearchandFollow()
	{
        ...
        if (Vector3.Distance(player.position, this.transform.position) > angle && ( Degreesight > Following))
        ...
        if (targeted == true) {
            // rotation degree sight.
            if (Vector3.Distance(player.position, this.transform.position) < Following && (angle < Degreesight))
            {
